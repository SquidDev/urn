	...
	elseif (sub1(esc1, 1, 2) == "_e") then
		...
		r_1611 = (function()
			if (pos1 <= len3) then
				...
				if (char2 == "_") then
					local r_1621 = list1(find1(esc1, "^_[%da-z]+_", pos1))
					local temp1
					local r_1641 = list_3f_1(r_1621)
					if r_1641 then
						local r_1651 = (_23_1(r_1621) == 2)
						if r_1651 then
							temp1 = true
						else
							temp1 = r_1651
					...
					if temp1 then
						local start1 = nth1(r_1621, 1)
						local _eend1 = nth1(r_1621, 2)
						pos1 = (pos1 + 1)
						local r_1681 = nil
						r_1681 = (function()
							if (pos1 < _eend1) then
...
remapError1 = (function(msg1)
	...
	local r_1461
	local r_1451
	r_1451 = gsub1(msg1, "local '([^']+)'", (function(x6)
		return _2e2e_1("local '", unmangleIdent1(x6), "'")
	end))
	r_1461 = gsub1(r_1451, "global '([^']+)'", (function(x7)
		return _2e2e_1("global '", unmangleIdent1(x7), "'")
	end))
	r_1471 = gsub1(r_1461, "upvalue '([^']+)'", (function(x8)
		return _2e2e_1("upvalue '", unmangleIdent1(x8), "'")
	end))
	res1 = gsub1(r_1471, "function '([^']+)'", (function(x9)
		return _2e2e_1("function '", unmangleIdent1(x9), "'")
	end))
	return res1
end)
remapMessage1 = (function(mappings1, msg2)
	local r_1481 = list1(match1(msg2, "^(.-):(%d+)(.*)$"))
	local temp2
	local r_1501 = list_3f_1(r_1481)
	if r_1501 then
		local r_1511 = (_23_1(r_1481) == 3)
		if r_1511 then
			temp2 = true
		else
			temp2 = r_1511
	...
	if temp2 then
		local file1 = nth1(r_1481, 1)
		local line1 = nth1(r_1481, 2)
		local extra1 = nth1(r_1481, 3)
...
generateMappings1 = (function(lines1)
	...
	iterPairs1(lines1, (function(line2, ranges1)
		...
		iterPairs1(ranges1, (function(pos2)
			...
			local r_1711 = pos2["finish"]["line"]
			local r_1691 = nil
			r_1691 = (function(r_1701)
				if (r_1701 <= r_1711) then
					if rangeList1[r_1701] then
					else
						rangeList1["n"] = succ1(rangeList1["n"])
						rangeList1[r_1701] = true
						if (r_1701 < rangeList1["min"]) then
							rangeList1["min"] = r_1701
						else
						end
						if (r_1701 > rangeList1["max"]) then
							rangeList1["max"] = r_1701
						else
						end
					end
