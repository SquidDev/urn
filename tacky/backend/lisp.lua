...
_ekey_3f_1 = (function(x9)
	return _e_3d3d_1(type1(x9), "key")
end);
type1 = (function(val1)
	local ty2
...
_e_3d_1 = (function(x10, y1)
	return _e_3d3d_1(x10, y1)
end);
succ1 = (function(x11)
	return _e_2b_1(1, x11)
end);
pred1 = (function(x12)
	return _e1(x12, 1)
end);
_eappend_21_1 = (function(writer1, text1)
	local r_971
	r_971 = type1(text1)
	if _e_7e3d_1(r_971, "string") then
		_eerror_21_1(format1("bad argment %s (expected %s, got %s)", "text", "string", r_971), 2)
	else
	end
	if _egetIdx1(writer1, "tabs-pending") then
		_esetIdx_21_1(writer1, "tabs-pending", false)
		_epushCdr_21_1(_egetIdx1(writer1, "out"), rep1("\t", _egetIdx1(writer1, "indent")))
	else
	end
	return _epushCdr_21_1(_egetIdx1(writer1, "out"), text1)
end);
_eline_21_1 = (function(writer2, text2)
	if text2 then
		_eappend_21_1(writer2, text2)
	else
	end
	if _egetIdx1(writer2, "tabs-pending") then
	else
		_esetIdx_21_1(writer2, "tabs-pending", true)
		return _epushCdr_21_1(_egetIdx1(writer2, "out"), "\n")
	end
end);
_eindent_21_1 = (function(writer3)
	return _esetIdx_21_1(writer3, "indent", succ1(_egetIdx1(writer3, "indent")))
end);
_eunindent_21_1 = (function(writer4)
	return _esetIdx_21_1(writer4, "indent", pred1(_egetIdx1(writer4, "indent")))
end);
_eestimateLength1 = (function(node1, max1)
	local tag2
	tag2 = _egetIdx1(node1, "tag")
	local _temp
	local r_851
	r_851 = _e_3d_1(tag2, "string")
	if r_851 then
		_temp = r_851
	else
		local r_861
		r_861 = _e_3d_1(tag2, "number")
		if r_861 then
			_temp = r_861
		else
			local r_871
			r_871 = _e_3d_1(tag2, "symbol")
			if r_871 then
				_temp = r_871
			else
				_temp = _e_3d_1(tag2, "key")
			end
		end
	end
	if _temp then
		return _e_23_s1(_enumber_3e_String1(_egetIdx1(node1, "contents")))
	elseif _e_3d_1(tag2, "list") then
		...
		r_1011 = (function()
			local _temp
			local r_1021
			r_1021 = _e_3c3d_1(sum1, max1)
			if r_1021 then
				_temp = _e_3c3d_1(i4, _e_23_2(node1))
			else
				_temp = r_1021
			end
			if _temp then
				sum1 = _e_2b_1(sum1, _eestimateLength1(nth1(node1, i4), _e1(max1, sum1)))
				if _e_3e_1(i4, 1) then
					sum1 = _e_2b_1(sum1, 1)
				else
				end
				i4 = _e_2b_1(i4, 1)
		...
		return fail1(_e_2e2e_1("Unknown tag ", tag2))
	end
end);
expression1 = (function(node2, writer5)
	...
	local _temp
	local r_881
	r_881 = _e_3d_1(tag3, "string")
	if r_881 then
		_temp = r_881
	else
		local r_891
		r_891 = _e_3d_1(tag3, "number")
		if r_891 then
			_temp = r_891
		else
			local r_901
			r_901 = _e_3d_1(tag3, "symbol")
			if r_901 then
				_temp = r_901
			else
				_temp = _e_3d_1(tag3, "key")
			end
		end
	end
	if _temp then
		return _eappend_21_1(writer5, _enumber_3e_String1(_egetIdx1(node2, "contents")))
	elseif _e_3d_1(tag3, "list") then
		_eappend_21_1(writer5, "(")
		if _enil_3f_1(node2) then
			return _eappend_21_1(writer5, ")")
		else
			local newline1, max2
			newline1 = false
			max2 = _e1(60, _eestimateLength1(car2(node2), 60))
			expression1(car2(node2), writer5)
			if _e_3c3d_1(max2, 0) then
				newline1 = true
				_eindent_21_1(writer5)
			else
			end
			local r_1051
			r_1051 = _e_23_2(node2)
			local r_1061
			r_1061 = 1
			local r_1031
			r_1031 = nil
			r_1031 = (function(r_1041)
				...
				if _temp then
					...
					local _temp
					local r_1071
					r_1071 = _e_21_1(newline1)
					if r_1071 then
						_temp = _e_3e_1(max2, 0)
					else
						_temp = r_1071
					end
					if _temp then
						max2 = _e1(max2, _eestimateLength1(entry1, max2))
						if _e_3c3d_1(max2, 0) then
							newline1 = true
							_eindent_21_1(writer5)
						else
			...
			r_1031(2)
			if newline1 then
				_eunindent_21_1(writer5)
			else
			end
			return _eappend_21_1(writer5, ")")
		end
	else
		return fail1(_e_2e2e_1("Unknown tag ", tag3))
	end
end);
block1 = (function(list1, writer6)
	...
	r_931 = (function(r_941)
		local _temp
		if _e_3c_1(0, 1) then
			_temp = _e_3c3d_1(r_941, r_951)
		else
			_temp = _e_3e3d_1(r_941, r_951)
		end
		if _temp then
			local r_911
			r_911 = r_941
			local node3
			node3 = _egetIdx1(r_921, r_911)
			expression1(node3, writer6)
			_eline_21_1(writer6)
			return r_931(_e_2b_1(r_941, r_961))
		else
		end
	end);
	return r_931(1)
