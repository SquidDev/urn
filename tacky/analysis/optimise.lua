...
removeNth_21_1 = (function(li1, idx1)
	local r_261 = type1(li1)
	if (r_261 ~= "list") then
		error1(format1("bad argment %s (expected %s, got %s)", "li", "list", r_261), 2)
	else
	end
	li1["n"] = (li1["n"] - 1)
...
visitQuote1 = (function(node1, visitor1, level1)
	if (level1 == 0) then
		...
		local _temp
		local r_1071 = (tag2 == "string")
		if r_1071 then
			_temp = r_1071
		else
			local r_1081 = (tag2 == "number")
			if r_1081 then
				_temp = r_1081
			else
				local r_1091 = (tag2 == "key")
				if r_1091 then
					_temp = r_1091
				else
					_temp = (tag2 == "symbol")
		...
		elseif (tag2 == "list") then
			...
			if _temp then
				local _temp
				local r_1111 = (first1["contents"] == "unquote")
				if r_1111 then
					_temp = r_1111
				else
...
visitNode1 = (function(node2, visitor2)
	if (visitor2(node2) == false) then
		...
		local _temp
		local r_1241 = (tag3 == "string")
		if r_1241 then
			_temp = r_1241
		else
			local r_1251 = (tag3 == "number")
			if r_1251 then
				_temp = r_1251
			else
				local r_1261 = (tag3 == "key")
				if r_1261 then
					_temp = r_1261
				else
					_temp = (tag3 == "symbol")
		...
		elseif (tag3 == "list") then
			...
			if _temp then
				...
				elseif (func1 == builtins1["quasiquote"]) then
					...
					local _temp
					local r_1321 = (func1 == builtins1["unquote"])
					if r_1321 then
						_temp = r_1321
					else
					...
					if _temp then
						...
						local _temp
						local r_1331 = (func1 == builtins1["define"])
						if r_1331 then
							_temp = r_1331
						else
						...
						elseif (funct1 == "macro") then
							...
							local _temp
							local r_1341 = (funct1 == "defined")
							if r_1341 then
								_temp = r_1341
							else
								local r_1351 = (funct1 == "arg")
								if r_1351 then
									_temp = r_1351
								else
									_temp = (funct1 == "native")
...
definitionsVisitor1 = (function(state5, node7)
	local _temp
	local r_941 = list_3f_1(node7)
	if r_941 then
		local r_951 = (_23_1(node7) > 0)
		if r_951 then
	...
	if _temp then
		...
		elseif (func2 == builtins2["set!"]) then
			...
			local _temp
			local r_1021 = (func2 == builtins2["define"])
			if r_1021 then
				_temp = r_1021
			else
...
definitionsVisit1 = (function(state6, nodes1)
	return visitBlock1(nodes1, 1, (function(r_1481)
		return definitionsVisitor1(state6, r_1481)
	end))
end)
	...
	local visit1 = (function(node8)
		if visited1[node8] then
			...
			if symbol_3f_1(node8) then
				...
				local _temp
				local r_1431 = list_3f_1(node8)
				if r_1431 then
					local r_1441 = (_23_1(node8) > 0)
					if r_1441 then
				...
				if _temp then
					...
					local _temp
					local r_1451 = (func3 == builtins2["set!"])
					if r_1451 then
						_temp = r_1451
					else
						local r_1461 = (func3 == builtins2["define"])
						if r_1461 then
							_temp = r_1461
						else
...
hasSideEffect1 = (function(node10)
	...
	local _temp
	local r_791 = (tag4 == "number")
	if r_791 then
		_temp = r_791
	else
		local r_801 = (tag4 == "string")
		if r_801 then
			_temp = r_801
		else
			local r_811 = (tag4 == "key")
			if r_811 then
				_temp = r_811
			else
				_temp = (tag4 == "symbol")
	...
	elseif (tag4 == "list") then
		...
		if (type1(fst1) == "symbol") then
			local var5 = fst1["var"]
			local r_821 = (var5 ~= builtins3["lambda"])
			if r_821 then
...
optimise1 = (function(nodes3)
	...
	r_831 = (function(r_841)
		...
		if _temp then
			...
			local _temp
			local r_871 = list_3f_1(node11)
			if r_871 then
				local r_881 = (_23_1(node11) > 0)
				if r_881 then
					local r_891 = symbol_3f_1(car2(node11))
					if r_891 then
	...
	r_901 = (function(r_911)
		...
		if _temp then
			...
			return r_901((r_911 + r_931))
		else
		end
	end)
	r_901(pred1(_23_1(nodes3)))
