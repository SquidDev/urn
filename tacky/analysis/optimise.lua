	...
	if (level1 == 0) then
		...
		elseif (tag2 == "list") then
			...
			if _temp then
				local _temp
				local r_1321
				r_1321 = (first1["contents"] == "unquote")
				if r_1321 then
					_temp = r_1321
				else
		...
		elseif (tag3 == "list") then
			...
			if _temp then
				...
				elseif (func1 == builtins1["quasiquote"]) then
					...
					local _temp
					local r_1531
					r_1531 = (func1 == builtins1["unquote"])
					if r_1531 then
						_temp = r_1531
					else
...
gatherDefinitions1 = (function(nodes1, lookup2)
	...
	return visitBlock1(nodes1, 1, (function(node5)
		local _temp
		local r_1571
		r_1571 = list_3f_1(node5)
		if r_1571 then
			local r_1581
			r_1581 = (_23_2(node5) > 0)
			if r_1581 then
				_temp = symbol_3f_1(car1(node5))
			else
				_temp = r_1581
			end
		else
...
gatherUsages1 = (function(nodes2, lookup3)
	...
	visit1 = (function(node6)
		if symbol_3f_1(node6) then
			...
			local _temp
			local r_1661
			r_1661 = list_3f_1(node6)
			if r_1661 then
				local r_1671
				r_1671 = (_23_2(node6) > 0)
				if r_1671 then
					_temp = symbol_3f_1(car1(node6))
				else
					_temp = r_1671
				end
			else
		...
		if _temp then
			...
			if r_1121 then
				...
				if r_1131 then
					...
					if r_1141 then
						_temp = (car1(node8)["var"] == builtins2["import"])
					else
						_temp = r_1141
					end
				else
