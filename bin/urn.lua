...
range_3f_1 = function(range)
	return type_23_1(range) == "table" and (range["tag"] == "range" and (range["name"] ~= nil and (range["start"] ~= nil and (range["finish"] ~= nil and range["lines"] ~= nil))))
end
...
rangeOfStart1 = function(range)
	local temp = type1(range)
	if temp ~= "range" then
		error1(format1("bad argument %s (expected %s, got %s)", "range", "range", temp), 2)
	end
	return {["tag"]="range", ["name"]=range["name"], ["start"]=range["start"], ["finish"]=range["start"], ["lines"]=range["lines"]}
end
rangeOfSpan1 = function(from, to)
	local temp = type1(from)
	if temp ~= "range" then
		error1(format1("bad argument %s (expected %s, got %s)", "from", "range", temp), 2)
	end
	local temp = type1(to)
	if temp ~= "range" then
		error1(format1("bad argument %s (expected %s, got %s)", "to", "range", temp), 2)
	end
...
compilerError_21_1 = function(message)
	local temp = type1(message)
	if temp ~= "string" then
		error1(format1("bad argument %s (expected %s, got %s)", "message", "string", temp), 2)
	end
...
child1 = function(parent, kind)
	if parent ~= nil then
		local temp = type1(parent)
		if temp ~= "scope" then
			error1(format1("bad argument %s (expected %s, got %s)", "parent", "scope", temp), 2)
		end
	end
	if kind == nil then
		kind = "normal"
...
var1 = function(name, kind, scope, node)
	...
	local temp = type1(scope)
	if temp ~= "scope" then
		error1(format1("bad argument %s (expected %s, got %s)", "scope", "scope", temp), 2)
	end
...
import_21_1 = function(scope, name, var, export)
	...
	local temp = type1(var)
	if temp ~= "var" then
		error1(format1("bad argument %s (expected %s, got %s)", "var", "var", temp), 2)
	end
		...
		if (i > n1(toks)) then
			...
			if kind == "mono" then
				if sub1(tok["whole"], 1, 3) == "```" then
					pushCdr_21_1(result, {["kind"]="text", ["contents"]="..."})
					break
				else
					pushCdr_21_1(result, tok)
					i = i + 1
				end
			elseif kind == "arg" or kind == "link" then
				...
				if _eend then
					pushCdr_21_1(result, {["kind"]=kind, ["contents"]=gsub1(sub1(tok["contents"], 1, _eend), "\n", " ") .. rep1("*", stars1[kind])})
					break
				else
					pushCdr_21_1(result, {["kind"]=kind, ["contents"]=gsub1(tok["contents"], "\n", " "), ["whole"]=tok["whole"] and gsub1(tok["whole"], "\n", " ")})
					i = i + 1
				end
			end
		end
...
eofError_21_1 = function(context, tokens, logger, msg, node, explain, ...)
	...
	if context then
		...
		return apply1(doNodeError_21_1, logger, msg, node, explain, lines)
	end
end
	...
	while offset <= length do
		...
		elseif find1(str, "^%-?%.?[#0-9]", offset) then
			...
			elseif char == "#" and lower1((function()
				...
				local str1 = apply1(_2e2e_1, split1(reverse1(sub1(str, start1, offset)), "'"))
		...
		elseif char == "\"" or char == "$" and (function()
			...
			if interpolate then
				...
				putNodeWarning_21_1(logger, "The $ syntax is deprecated and should be replaced with format.", range(start, {["tag"]="position", ["offset"]=offset, ["line"]=line, ["column"]=column}), nil, range(start, {["tag"]="position", ["offset"]=offset, ["line"]=line, ["column"]=column}), "Can be replaced with (format nil " .. quoted1(concat1(sections)) .. ")")
...
parse1 = function(logger, toks, cont)
	...
	while temp1 <= temp do
		...
		elseif tag == "interpolate" then
			local node = {["tag"]="list", ["n"]=2, ["range"]=tok["range"], [1]={["tag"]="symbol", ["contents"]="$", ["range"]=rangeOfStart1(tok["range"])}, [2]={["tag"]="string", ["value"]=tok["value"], ["range"]=tok["range"]}}
		...
		elseif tag == "close" then
			...
			elseif head["close"] ~= tok["contents"] then
				...
				head["range"] = rangeOfSpan1(head["range"], tok["range"])
				pop_21_()
			end
			...
			elseif tag == "list" then
				...
				if type1(node) == "symbol" then
					...
					elseif first["contents"] == "syntax-quote" then
						...
						local temp1 = 1
						while temp1 <= temp do
							visitQuote2(node[temp1], level, lookup)
							temp1 = temp1 + 1
						end
						return nil
					end
				...
				if type1(first) == "list" and builtin_3f_1(car1(first), "lambda") then
					...
					while temp3 <= temp2 do
						...
						if val == nil or builtin_3f_1(val, "nil") then
							lookup[nth1(args, temp3)["var"]] = {["tag"]="rec-func", ["parent"]=node, ["setter"]=nil, ["lambda"]=nil, ["recur"]=0, ["direct"]=0, ["var"]=0}
						end
						temp3 = temp3 + 1
					end
					if parents then
...
visitNodes1 = function(node, start, parents, lookup)
	...
	local temp1 = start
	while temp1 <= temp do
		visitNode2(nth1(node, temp1), parents, nil, lookup)
		temp1 = temp1 + 1
	end
	return nil
end
...
visitRecur1 = function(lookup, recur)
	...
	local temp1 = 3
	while temp1 <= temp do
		nodeCaptured1(nth1(lam, temp1), allCaptured, recBoundary)
		temp1 = temp1 + 1
	end
...
escapeVar1 = function(var, state)
	if builtinVars1[var] then
		...
		local esc = state["var-lookup"][var]
		if not esc then
			formatOutput_21_1(1, "" .. (var["name"]) .. " hs not been escaped (at " .. (function()
				local x
				if var["node"] then
					x = formatNode1(var["node"])
				else
					x = "?"
				end
				return x
...
compileExpression1 = function(node, out, state, ret, _ebreak)
	...
	elseif catTag == "call-tail" then
		...
		elseif n1(zipped) == 1 and empty_3f_1(caar1(zipped)) then
			...
			local temp2 = 1
			while temp2 <= temp1 do
				compileExpression1(temp[temp2], out, state, "")
				line_21_1(out)
				temp2 = temp2 + 1
			end
		else
			...
			local first, temp = true, n1(zipped)
			local temp1 = 1
			while temp1 <= temp do
				local zip = zipped[temp1]
				local args = car1(zip)
			...
			while temp1 <= temp do
				...
				if any1(symVariadic_3f_1, args) then
					...
					while temp3 <= temp2 do
						...
						compileExpression1(nth1(vals, temp3), out, state)
						temp3 = temp3 + 1
					end
					append_21_1(out, ")")
			...
			elseif n1(car1(packZip)) == 1 then
				...
				if varIdx > 1 then
					...
					while temp1 <= temp do
						...
						append_21_1(out, escapeVar1(mapVar1(mapping, nth1(args, temp1)["var"]), state))
						temp1 = temp1 + 1
					end
					append_21_1(out, " = ")
					local temp = varIdx - 1
		...
		elseif car1(args1)["var"]["is-variadic"] or n1(args1) > 1 and any1(symVariadic_3f_1, args1) then
			if n1(args1) > 1 then
				...
				while temp1 <= temp do
					...
					append_21_1(out, pushEscapeVar_21_1(nth1(args1, temp1)["var"], state))
					temp1 = temp1 + 1
				end
...
compileBindVariadic1 = function(out, args, vals, state, mapping)
	...
	if varIdx < nargs then
		...
		while temp <= nargs do
			...
			append_21_1(out, escapeVar1(mapVar1(mapping, nth1(args, temp)["var"]), state))
			temp = temp + 1
		end
		append_21_1(out, " = ")
		local temp = varIdx + 1
		...
		visit = function(state1, stack, stackHash)
			...
			if idx then
				if state1["var"]["kind"] == "macro" then
					...
					return doNodeError_21_1(state1["logger"], "Loop in macros " .. concat1(states, " -> "), firstNode, nil, splice1(nodes))
				else
					return nil
				end
...
getNative1 = function(meta)
	if not meta["has-value"] then
		...
		compileNative1(out, meta)
		meta["has-value"] = true
		local fun = load1(concat1(out["out"]), "=" .. meta["name"])
		if fun then
...
expectType_21_1 = function(log, node, parent, expectedType, name)
	if type1(node) ~= expectedType then
		local node1, message, extra = node or parent, "Expected " .. (name or expectedType) .. ", got " .. (function()
			if node then
				return type1(node)
			else
				return "nothing"
	...
	while true do
		...
		elseif temp == "symbol" then
			...
			if node1["var"]["kind"] == "builtin" then
				doNodeError_21_1(state["logger"], "Cannot have a raw builtin.", node1, nil, getSource1(node1), "")
			end
		...
		elseif temp == "list" then
			...
			if temp1 == "symbol" then
				...
				if temp2 == "builtin" then
					if func == builtins1["lambda"] then
						expectType_21_1(state["logger"], nth1(node1, 2), node1, "list", "argument list")
					...
					elseif func == builtins1["cond"] then
						local temp3 = n1(node1)
						local temp4 = 2
						while temp4 <= temp3 do
							local case = nth1(node1, temp4)
							expectType_21_1(state["logger"], case, node1, "list", "case expression")
						...
						return node1
					elseif func == builtins1["set!"] then
						expectType_21_1(state["logger"], nth1(node1, 2), node1, "symbol")
						expect_21_1(state["logger"], nth1(node1, 3), node1, "value")
						maxLength_21_1(state["logger"], node1, 3, "set!")
						local var = lookupAlways_21_1(scope, nth1(node1, 2)["contents"], nth1(node1, 2))
						require_21_1(state, var, nth1(node1, 2))
						node1[2]["var"] = var
						if var["const"] then
							doNodeError_21_1(state["logger"], format1("Cannot rebind immutable definition '%s'", nth1(node1, 2)["contents"]), node1, format1("Top level definitions are immutable by default. If you want\nto redefine '%s', add the `:mutable` modifier to its definition.", nth1(node1, 2)["contents"]), getSource1(node1), "")
						end
						node1[3] = resolveNode1(nth1(node1, 3), scope, state)
						return node1
					elseif func == builtins1["quote"] then
						expect_21_1(state["logger"], nth1(node1, 2), node1, "value")
						maxLength_21_1(state["logger"], node1, 2, "quote")
						return node1
					elseif func == builtins1["syntax-quote"] then
						expect_21_1(state["logger"], nth1(node1, 2), node1, "value")
						maxLength_21_1(state["logger"], node1, 2, "syntax-quote")
					...
					elseif func == builtins1["unquote"] then
						...
						while temp4 <= temp3 do
							...
							local built = resolveNode1(nth1(node1, temp4), scope, childState)
							built_21_1(childState, {["tag"]="list", ["n"]=3, ["range"]=built["range"], ["owner"]=built["owner"], ["parent"]=node1, [1]={["tag"]="symbol", ["contents"]="lambda", ["var"]=builtins1["lambda"]}, [2]={tag="list", n=0}, [3]=built})
							...
							elseif type1(temp5) == "list" and (n1(temp5) >= 1 and (nth1(temp5, 1) == true and true)) then
								...
								elseif n1(replacement) == 1 then
									...
									local log, node2, message, extra = state["logger"], nth1(node1, temp4), "Expected one value, got " .. n1(replacement)
									doNodeError_21_1(log, message, node2, extra, getSource1(node2), "")
								end
							else
						...
						local temp4 = 1
						while temp4 <= temp3 do
							result[temp4] = resolveExecuteResult1(nth1(states, temp4), nth1(result, temp4), node1, scope, state)
							temp4 = temp4 + 1
						end
						if n1(result) == 1 then
							node = car1(result)
						elseif many then
							...
							return doNodeError_21_1(state["logger"], "Multiple values returned in a non block context", node1, nil, getSource1(node1), "")
						end
					elseif func == builtins1["unquote-splice"] then
						maxLength_21_1(state["logger"], node1, 2, "unquote-splice")
						local childState = create3(scope, state["compiler"])
						...
						elseif type1(temp3) == "list" and (n1(temp3) >= 1 and (nth1(temp3, 1) == true and true)) then
							...
							local temp5 = 1
							while temp5 <= temp4 do
								result[temp5] = resolveExecuteResult1(childState, nth1(result, temp5), node1, scope, state)
								temp5 = temp5 + 1
							end
							if n1(result) == 1 then
								node = car1(result)
					...
					elseif func == builtins1["define"] then
						...
						expectType_21_1(state["logger"], nth1(node1, 2), node1, "symbol", "name")
						expect_21_1(state["logger"], nth1(node1, 3), node1, "value")
						local var = addVerbose_21_1(scope, nth1(node1, 2)["contents"], "defined", node1, state["logger"])
						var["display-name"] = (nth1(node1, 2)["display-name"])
						define_21_1(state, var)
						node1["def-var"] = var
						handleMetadata1(state["logger"], node1, var, 3, n1(node1) - 1)
					...
					elseif func == builtins1["define-macro"] then
						...
						expectType_21_1(state["logger"], nth1(node1, 2), node1, "symbol", "name")
						expect_21_1(state["logger"], nth1(node1, 3), node1, "value")
						local var = addVerbose_21_1(scope, nth1(node1, 2)["contents"], "macro", node1, state["logger"])
						var["display-name"] = (nth1(node1, 2)["display-name"])
						define_21_1(state, var)
						node1["def-var"] = var
						handleMetadata1(state["logger"], node1, var, 3, n1(node1) - 1)
					...
					elseif func == builtins1["define-native"] then
						...
						expectType_21_1(state["logger"], nth1(node1, 2), node1, "symbol", "name")
						local var = addVerbose_21_1(scope, nth1(node1, 2)["contents"], "native", node1, state["logger"])
						var["display-name"] = (nth1(node1, 2)["display-name"])
						define_21_1(state, var)
						node1["def-var"] = var
						handleMetadata1(state["logger"], node1, var, 3, n1(node1))
						return node1
					elseif func == builtins1["import"] then
						expectType_21_1(state["logger"], nth1(node1, 2), node1, "symbol", "module name")
						...
						elseif temp3 == "key" then
							...
							expectType_21_1(state["logger"], nth1(node1, 3), node1, "symbol", "alias name of import list")
						end
						maxLength_21_1(state["logger"], node1, exportIdx, "import")
						yield1({["tag"]="import", ["module"]=nth1(node1, 2)["contents"], ["as"]=as, ["symbols"]=symbols, ["export"]=(function()
							local export = nth1(node1, exportIdx)
							if export then
								expectType_21_1(state["logger"], export, node1, "key", "import modifier")
					...
					elseif func == builtins1["struct-literal"] then
						if n1(node1) % 2 ~= 1 then
							local log, message, extra = state["logger"], "Expected an even number of arguments, got " .. n1(node1) - 1
							doNodeError_21_1(log, message, node1, extra, getSource1(node1), "")
						end
						return resolveList1(node1, 2, scope, state)
					else
						return errorInternal_21_1(state["logger"], node1, "Unknown builtin " .. (function()
							if func then
								return func["name"]
							else
								return "?"
			...
			elseif temp1 == "list" then
				...
				local log, node2, message, extra = state["logger"], first or node1, "Cannot invoke a non-function type '" .. temp1 .. "'"
				return doNodeError_21_1(log, message, node2, extra, getSource1(node2), "")
			end
		else
			return error1("Pattern matching failure!\nTried to match the following patterns against " .. pretty1(temp) .. ", but none matched.\n" .. "  Tried: `\"number\"`\n  Tried: `\"string\"`\n  Tried: `\"key\"`\n  Tried: `\"symbol\"`\n  Tried: `\"list\"`")
		end
...
compile1 = function(compiler, nodes, scope, name)
	...
	while n1(queue) > 0 and skipped <= n1(queue) do
		...
		self1(logger, "put-debug!", (type1(head) .. " for " .. head["_state"]["stage"] .. " at " .. formatNode1(head["_node"]) .. " (" .. (function()
	...
	return splice1(list1((map2(function(temp)
		return temp["node"]
	end, states)), states))
end
pathEscape1 = {["?"]="(.*)", ["."]="%.", ["%"]="%%", ["^"]="%^", ["$"]="%$", ["+"]="%+", ["-"]="%-", ["*"]="%*", ["["]="%[", ["]"]="%]", ["("]="%)", [")"]="%)"}
...
formatSignature1 = function(name, var)
	...
	if sig == nil then
		...
		return "(" .. concat1(cons1(name, sig), " ") .. ")"
	end
end
formatLink1 = function(name, var, title)
	local loc, sig = gsub1(stripExtension1(((getSource1((var["node"])))["name"])), "/", "."), extractSignature1(var)
	local hash
	if sig == nil then
		hash = var["name"]
...
index1 = function(out, libraries)
	...
	local temp = n1(libraries)
	local temp1 = 1
	while temp1 <= temp do
		local lib = libraries[temp1]
		local temp2 = lib["scope"]["exported"]
		...
		while temp3 ~= nil do
			...
			if not info then
				...
				pushCdr_21_1(lookup, info)
			end
			if var["scope"] == lib["scope"] then
				info["defined"] = lib
			else
				pushCdr_21_1(info["exported"], list1(temp3, lib))
			end
	...
	local temp = n1(letterList)
	local temp1 = 1
	while temp1 <= temp do
		local xs, f = cadr1((letterList[temp1])), function(a, b)
	...
	while temp1 <= temp do
		...
		local temp2 = cadr1(letter)
		local temp3 = n1(temp2)
		local temp4 = 1
		while temp4 <= temp3 do
			local info = temp2[temp4]
			local var, defined = info["var"], info["defined"]
			local range = getSource1(var["node"])
			append_21_1(out, "| |")
			...
			if empty_3f_1(info["exported"]) then
				...
				append_21_1(out, format1("[%s](%s.md \"Also exported from %s\")", name, path, concat1(sort2(nub1(map2(function(x)
					return cadr1(x)["name"]
				end, info["exported"]))), ", ")))
			end
			line_21_1(out, "|")
...
genNative1 = function(compiler, args)
	...
	local prefix, lib = args["gen-native"], compiler["lib-cache"][last1(args["input"])]
	local escaped
	if string_3f_1(prefix) then
...
repl1 = function(compiler, args)
	...
	local temp2 = 1
	while temp2 <= temp1 do
		local library = car1(pathLoader1(compiler, (temp[temp2])))
	...
	while temp2 <= temp1 do
		...
		while temp3 <= nLines do
			...
			if not isActive and count > 0 then
				self1(logger, "put-warning!", (formatOutput_21_1(nil, "" .. path .. ":" .. temp3 .. " is not active but has count " .. count)))
			end
...
visitEagerExported_3f_1 = function(val, _5f_, node)
	...
	return temp or (not (type1(val) == "list") or not builtin_3f_1(car1(val), "lambda"))
end
...
transformer1 = {["name"]="transformer", ["help"]="Run the given TRANSFORMERS on the provides NODES with the given\nLOOKUP information.", ["cat"]={tag="list", n=2, "opt", "usage"}, ["run"]=function(temp, state, nodes, lookup, transformers)
	...
	while temp2 <= temp1 do
		...
		if state["track"] then
			self1(state["logger"], "put-verbose!", (sprintf1("%s made %d changes", "[" .. concat1(nth1(transformers, temp2)["cat"], " ") .. "] " .. nth1(transformers, temp2)["name"], nth1(trackers, temp2)["changed"])))
		end
		temp2 = temp2 + 1
	end
	return nil
		...
		elseif type1(node) == "list" then
			...
			if type1(first) == "symbol" then
				...
				elseif first["contents"] == "syntax-quote" then
					...
					local temp1 = 1
					while temp1 <= temp do
						visitQuote4(defined, logger, node[temp1], level)
						temp1 = temp1 + 1
					end
					return nil
				end
...
visitBlock3 = function(defined, logger, node, start, last)
	...
	local temp1 = start
	while temp1 <= temp do
		visitNode4(defined, logger, nth1(node, temp1))
		temp1 = temp1 + 1
	end
	if n1(node) >= start then
...
unusedVars1 = {["name"]="unused-vars", ["help"]="Ensure all non-exported NODES are used.", ["cat"]={tag="list", n=1, "warn"}, ["level"]=2, ["run"]=function(temp, state, _5f_, lookup)
	...
	sort1(unused, function(node1, node2)
		return range_3c_1(getSource1(cadr1(node1)), getSource1(cadr1(node2)))
	end)
	...
	while temp2 <= temp1 do
		...
		if var then
			...
			if not var["const"] and (n1(info["defs"]) == 1 and (function()
				...
				putNodeWarning_21_1(state["logger"], format1("%s is never mutated", quoted1(var["name"])), node, "This definition is explicitly marked as :mutable, but is\nnever mutated. Consider removing the annotation.", getSource1(node), "variable defined here")
			end
		end
		temp2 = temp2 + 1
	end
...
putLines_21_1 = function(ranges, entries)
	...
	local previous, file, code, temp = -1, nth1(entries, 1)["name"], coloured1("32;1", " %" .. n1(tostring1((reduce1(function(max, range)
	...
	local temp1 = 1
	while temp1 <= temp do
		local range, message = entries[temp1], entries[temp1 + 1]
...
includeRocksImpl1 = function(logger, paths, cfg, path, manif, vers, util, dir)
	...
	local temp2 = 1
	while temp2 <= temp1 do
		local tree = temp[temp2]
		local temp3 = call1(manif, "load_local_manifest", (call1(path, "rocks_dir", tree)))["repository"]
		local temp4, versions = next1(temp3)
		while temp4 ~= nil do
			if not (not next1(versions) or temp4 == "urn") then
				local mainVersion = apply1(min1, ((function()
					local temp5 = keys1(versions)
					return map2(vers["parse_version"], temp5)
				end)()))["string"]
				local rootPath = call1(path, "install_dir", temp4, mainVersion, tree)
				self1(logger, "put-verbose!", (format1("Including %s %s (located at %s)", temp4, mainVersion, rootPath)))
				pushCdr_21_1(paths, call1(dir, "path", rootPath, "urn-lib", "?"))
				pushCdr_21_1(paths, call1(dir, "path", rootPath, "urn-lib", "?", "init"))
			end
			temp4, versions = next1(temp3, temp4)
		end
		temp2 = temp2 + 1
	end
	return nil
...
createPluginState1 = function(compiler)
	...
	return {["logger/put-error!"]=function(temp)
		...
		local lines = _pack(...) lines.tag = "list"
		return putNodeError_21_1(logger, msg, node, explain, splice1(lines))
	end, ["logger/put-node-warning!"]=function(msg, node, explain, ...)
		local lines = _pack(...) lines.tag = "list"
		return putNodeWarning_21_1(logger, msg, node, explain, splice1(lines))
	end, ["logger/do-node-error!"]=function(msg, node, explain, ...)
		local lines = _pack(...) lines.tag = "list"
		return doNodeError_21_1(logger, msg, node, explain, splice1(lines))
